...
async function searchWeebverseOne(input, numOfSearch) {
  ...
  const results = await Promise.all(mangaList
    .filter(manga => manga.s.toLowerCase().includes(input.toLowerCase()))
    .slice(0, numOfSearch)
    .map(async manga => {
      const mangaUrl = `https://manga4life.com/manga/${manga.i}`;
      const { data: mangaData } = await axios.get(mangaUrl);
      const $manga = cheerio.load(mangaData);

      const authors = $manga('.list-group-item:contains("Author(s):")').find('a').map((_, el) => $manga(el).text()).get().join(', ');
      const description = $manga('.list-group-item:contains("Description:")').find('.Content').text().trim();
